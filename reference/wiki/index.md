...
- [Remote Compose POC 회고와 학습 포인트](remote-compose-poc-retrospective.md) — Android Compose API 대응표, 상태 소유권, density·state 트러블슈팅, 기존 앱 도입 체크리스트와 제품 판단
- [테스트와 운영](testing-and-operations.md) — 호환성, golden, fuzz, 접근성, 관측성
- [파일럿 계획](pilot-plan.md) — 단계별 PoC와 go/no-go 기준
- [Ktor → Android SDUI POC](sample-sdui-poc.md) — native 직접 입력, server-backed 가변 목록, Remote Compose scroll·상세 Navigation, 자동 reload
